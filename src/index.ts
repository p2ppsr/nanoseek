...
export async function resolve(query: { UHRPUrl: string; confederacyHost?: string; clientPrivateKey?: string }): Promise<string[]> {
  ...
  try {
    ...
    const jsonString = typeof response.body === 'string' 
      ? response.body 
      : Buffer.from(response.body).toString('utf-8')
    lookupResult = JSON.parse(jsonString)
  } catch (error) {
    console.error('Error parsing Confederacy response:', error)
    if (error instanceof SyntaxError) {
      throw new NanoSeekError('Confederacy lookup failed: Invalid JSON response', 'ERR_INVALID_JSON')
    } else {
      throw new NanoSeekError('Confederacy lookup failed: Unexpected error', 'ERR_CONFEDERACY_LOOKUP')
    }
  }
  ...
  if (!Object.prototype.hasOwnProperty.call(lookupResult[0], 'outputScript')) {
    throw new NanoSeekError('Invalid response format from Confederacy', 'ERR_INVALID_RESPONSE')
  }
